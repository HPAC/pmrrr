      SUBROUTINE ODRRV( N, VL, VU, D, L, PIVMIN,
...
*  The input eigenvalues should have been computed by ODRRE.
*
*  Arguments
*  =========
*
...
*          is stored the corresponding shift as given by ODRRE.
...
*          W from ODRRE is expected here ). Furthermore, they are with
...
*     .. External Subroutines ..
      EXTERNAL           ODCPY, ODR1V, ODRRB, ODRRF, ODSET,
     $                   ODSCL
*     ..
*     .. Intrinsic Functions ..
...
*     The width of the part of Z that is used
      ...
      CALL ODSET( 'Full', N, ZUSEDW, ZERO, ZERO,
     $                    Z(1,ZUSEDL), LDZ )
...
*                       Get representation from the right end of Z array
                     ...
                     ENDIF
                  ENDIF
                  CALL ODCPY( IN, Z( IBEGIN, J ), 1, D( IBEGIN ), 1 )
                  CALL ODCPY( IN-1, Z( IBEGIN, J+1 ), 1, L( IBEGIN ),
     $               1 )
                  SIGMA = Z( IEND, J+1 )

*                 Set the corresponding entries in Z to zero
                  CALL ODSET( 'Full', IN, 2, ZERO, ZERO,
     $                         Z( IBEGIN, J), LDZ )
               END IF
...
*
C                    ODRRF needs LWORK = 2*N
                     CALL ODRRF( IN, D( IBEGIN ), L( IBEGIN ),
     $                         WORK(INDLD+IBEGIN-1),
...
*                    Given LAMBDA, compute the eigenvector.
                     CALL ODR1V( IN, 1, IN, LAMBDA, D( IBEGIN ),
