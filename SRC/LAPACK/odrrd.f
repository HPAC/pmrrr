      SUBROUTINE ODRRD( RANGE, ORDER, N, VL, VU, IL, IU, GERS,
...
*
      IF( IRANGE.LE.0 ) THEN
         INFO = -1
      ELSE IF( .NOT.(OLSAME(ORDER,'B').OR.OLSAME(ORDER,'E')) ) THEN
         INFO = -2
      ELSE IF( N.LT.0 ) THEN
         INFO = -3
...
*           Find negcount of initial interval boundaries GL and GU
            WORK( N+1 ) = GL
            WORK( N+IN+1 ) = GU
            CALL ODEBZ( 1, 0, IN, IN, 1, NB, ATOLI, RTOLI, PIVMIN,
...
*           Compute Eigenvalues
     ...
     $              LOG( TWO ) ) + 2
            CALL ODEBZ( 2, ITMAX, IN, IN, 1, NB, ATOLI, RTOLI, PIVMIN,
     $                   D( IBEGIN ), E( IBEGIN ), E2( IBEGIN ),
...
*     If ORDER='E', sort the eigenvalues from smallest to largest
      ...
      IF( OLSAME(ORDER,'E') .AND. NSPLIT.GT.1 ) THEN
         DO 150 JE = 1, M - 1
            IE = 0
            TMP1 = W( JE )
            DO 140 J = JE + 1, M
