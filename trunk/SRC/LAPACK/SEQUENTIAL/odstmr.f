      SUBROUTINE ODSTMR( JOBZ, RANGE, N, D, E, VL, VU, IL, IU,
...
*     .. External Functions ..
      LOGICAL            OLSAME
      DOUBLE PRECISION   ODMCH, ODNST
      EXTERNAL           OLSAME, ODMCH, ODNST
*     ..
*     .. External Subroutines ..
      EXTERNAL           ODCPY, ODE2, ODEV2, ODRRC, ODRRE, ODRRJ,
     $                   ODRRR, ODRRV, ODSRT, ODSCL, ODSWAP, OERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      INFO = 0
      IF( .NOT.( WANTZ .OR. OLSAME( JOBZ, 'N' ) ) ) THEN
         INFO = -1
...
*
         ...
         ELSE IF( WANTZ .AND. VALEIG ) THEN
            CALL ODRRC( 'T', N, VL, VU, D, E, SAFMIN,
...
*
      IF( N.EQ.2 ) THEN
         IF( .NOT.WANTZ ) THEN
            CALL ODE2( D(1), E(1), D(2), R1, R2 )
         ELSE IF( WANTZ.AND.(.NOT.ZQUERY) ) THEN
            CALL ODEV2( D(1), E(1), D(2), R1, R2, CS, SN )
         END IF
         IF( ALLEIG.OR.
...
*     comments in ODRRD.  The preference for scaling small values
...
*
      ...
      IF( SCALE.NE.ONE ) THEN
         CALL ODSCL( N, SCALE, D, 1 )
         CALL ODSCL( N-1, SCALE, E, 1 )
         TNRM = TNRM*SCALE
         IF( VALEIG ) THEN
...
*        Note: these settings do only affect the subset case and ODRRE
      ...
      ENDIF
      CALL ODRRE( RANGE, N, WL, WU, IIL, IIU, D, E,
